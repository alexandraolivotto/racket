...
; pe cea di n urmă. Colțul din stânga sus a păsării se va află inițial la:
;    y = bird-inițial-y
; și x = bird-x.
; (get-initial-state) va fi o funcție care va returna starea inițială a jocului.
(define-struct bird-struct (x y v-y) #:transparent)

(define-struct pipe (gap-x gap-y) #:transparent)

(define-struct state (bird-struct pipe-list score) #:transparent)
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-struct _ y v-y) bird])
   (struct-copy bird-struct bird [y (+ v-y y)] [v-y (+ v-y gravity)] )))
...
(define (change current-state pressed-key)
    (match-let ([(state bird _ _) current-state])
    (cond [(key=? pressed-key " ")
          (struct-copy state current-state [bird-struct (next-state-bird-onspace bird initial-momentum)])]
          [else current-state])))
